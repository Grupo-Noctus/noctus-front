<template>
    ...
    <v-row justify="center">
        ...
        <v-col v-else-if="!courses || courses.length === 0" cols="12" class="text-center">
            <v-alert type="info" class="w-75 custom-alert">Nenhum curso encontrado em sua matricula.</v-alert>
        </v-col>

        <v-col
            v-for="course in courses"
            v-else
            :key="course.idEnrrolment"
            cols="auto"
            sm="6"
            md="4"
            lg="4"
            ...
            >
                ...
                <div class="container-subtitle">
                    <v-card-subtitle class="mb-3 text-body-2">
                        Expira em:
                        <span :class="isExpired(course.expiresAt) ? 'text-red' : ''">{{
                ...
                <v-card-actions>
                    <v-btn
                        ...
                        size="large"
                        block
                        @click="viewCourse(course.idCourse)"
                    >
                        Acessar Curso
                    </v-btn>
                </v-card-actions>
